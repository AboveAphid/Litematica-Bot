// Credit, this script was created by "y3621555" on github (also known as Johnson).
// Some of the console.logs chinese have been replaced with English translations (through the use of Google Translate)
// This code has also been modified by AboveAphid (github)
...
const default_max_fly_speed = 20.0
const default_max_walk_speed = 20.0

const legitimate_block = ['air', 'cave_air', 'light', 'water'];
const astar_pathfinder = {
    /**
     * A* Search 並限制可循路區塊
     * @param {Vec3} target - 目標座標.
     */
    astarfly: async function (bot, target, walkSpeed, flySpeed, lastFlyTime, mute) {  //直接使用方塊座標
        const mcData = require('minecraft-data')(bot.version)
        mute = (mute === undefined ? false : mute)
        let distance = bot.entity.position.distanceTo(target);
        flySpeed = (flySpeed === undefined ? default_max_fly_speed : flySpeed)
        walkSpeed = (walkSpeed === undefined ? default_max_walk_speed : walkSpeed)
        ...
        while (true) {// \x1b[33m
            ...
            if (!alreadyCheckTargetNoObstacle && bot.blockAt(target) != null && bot.blockAt(target.offset(0, 1, 0)) != null) {//進加載區後判斷是否被阻擋
                ...
                if (legitimate_block.indexOf(bot.blockAt(target).name) == -1 || legitimate_block.indexOf(bot.blockAt(target.offset(0, 1, 0)).name) == -1) {
                    ...
                    console.log(`The target is blocked. Finding a replacement spot. ${bot.blockAt(target).name} ${bot.blockAt(target.offset(0, 1, 0)).name}.`)
                    ...
                    // console.log(`Determined matching type: ${matchingType}`)
                    ...
                    if (secondPos == null) {
                        ...
                        console.log(`[Warning] ${secondTargetMaxDistance++} grid cannot find substitute coordinates. Increase the search range and try again.`)
                        alreadyCheckTargetNoObstacle = false;
                    } else {
                        // console.log(`替換終點座標 ${target} -> ${secondPos.position}`)
                        console.log(`Replaced the end point coordinates: ${target} -> ${secondPos.position}`)
            ...
            //     moveError=0;
            // }
            await astar(locNow, target);
            astar_timer++;
        }
        async function astar(locNow, target) {
            ...
            while (OPEN.length > 0) {
                ...
                if (Date.now() - astar_start_time > 3000) {
                    ...
                    console.log('\x1b[31mA*timeout\x1b[0m wait 30 ms');
                    await sleep(30);
                    break;
                }
...
module.exports = {astar_pathfinder}